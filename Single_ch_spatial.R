...
#' Base directory
wd = r"(C:\Users\mf420\UK Dementia Research Institute Dropbox\Brancaccio Lab\Brancaccio Lab manuscripts in preparation\ClockCyte\Fig3\4.15_baseline_GCaMP8s\hemis\Left)"
wd = back_to_forw(wd)
...
for (i in seq(from = 1, to = length(files))){
  ...
  ch_grid_vals_path = file.path(foldername, "Ch2_2_grid_vals.csv")
  print("Importing Channel grid vals")
  ch_grid_vals = importGridVals(ch_grid_vals_path)
  ...
  ch1_cells = ch_grid_vals[within_scn, ]
  ...
  far_circ <- circular_plot(data = sub1$phase_rad, path = file.path(newdir, paste0(filename, "_close_cells.svg")), saving = saving)
  
  # isolate variance components and plot
  newrecord = c(far_vec_len = far_circ$vec_len,
